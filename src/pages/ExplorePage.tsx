...
const categories: { name: string; icon: typeof Paintbrush; colorClass: string; activeClass: string }[] = [
  { name: 'Murals', icon: Paintbrush, colorClass: 'text-primary', activeClass: 'bg-primary' },
  { name: 'Cafes', icon: Coffee, colorClass: 'text-accent', activeClass: 'bg-accent' },
  { name: 'Parks', icon: TreePine, colorClass: 'text-secondary', activeClass: 'bg-secondary' },
  { name: 'Viewpoints', icon: Binoculars, colorClass: 'text-primary', activeClass: 'bg-primary' },
  { name: 'Markets', icon: ShoppingBasket, colorClass: 'text-destructive', activeClass: 'bg-destructive' },
  { name: 'Galleries', icon: Image, colorClass: 'text-accent', activeClass: 'bg-accent' },
  { name: 'Gardens', icon: Flower2, colorClass: 'text-secondary', activeClass: 'bg-secondary' },
  { name: 'Historic', icon: Landmark, colorClass: 'text-accent', activeClass: 'bg-accent' },
];
...
export default function ExplorePage() {
  ...
  return (
    <div className="min-h-screen bg-background pb-24">
      ...
      {/* Category grid — matching SwiftUI categoryCard */}
        ...
        {categories.map(({ name, icon: Icon, colorClass, activeClass }) => {
          ...
          return (
            <button
              ...
              className={`flex items-center gap-2.5 p-3.5 rounded-2xl text-left transition-all border ${
                isSelected
                  ? `${activeClass} text-primary-foreground border-transparent`
                  : 'ios-card border-border/30'
              }`}
            >
